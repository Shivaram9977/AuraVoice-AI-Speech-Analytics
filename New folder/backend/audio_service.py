...
class AudioService:
    ...
    def generate_spectrogram(self, audio_bytes: bytes, filename: str) -> str:
        ...
        try:
            ...
            ax.set_facecolor('#1a1b23')
            
            # Plot the spectrogram
            img = librosa.display.specshow(s_mel_db, x_axis='time', y_axis='mel', sr=sr_rate, fmax=8000, ax=ax, cmap='magma')
            
            # Design customization to match glassmorphism theme
            ax.tick_params(colors='#8f9cae', labelsize=8)
            ax.yaxis.label.set_color('#8f9cae')
            ax.xaxis.label.set_color('#8f9cae')
            
            # Hide borders
            for spine in ax.spines.values():
                spine.set_edgecolor('#313543')

            plt.title('Voice Frequency Spectrum (Mel-Spectrogram)', color='#ffffff', fontsize=10, pad=10)
            plt.tight_layout()
